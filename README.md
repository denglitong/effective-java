...
85. 其他方法优先于 Java 序列化
    1. 避免序列化攻击的最佳方式是永远不要反序列化任何东西
    2. 在新编写的任何新系统中都没有理由再使用Java序列化
    3. 永远不要反序列化不被信任的数据
    4. 白名单由于黑名单（最小权限）

86. 谨慎地实现 Serializable 接口
    1. 实现 Serializable 接口而付出的最大代价是，一旦一个类被发布，就大大降低了"改变这个类的实现"的灵活性
    2. 实现 Serializable 的第三个代价是，随着发行版的版本，相关的测试负担也会增加
    3. 实现 Serializable 接口并不是一个很轻松就可以做出的决定
    4. 为了继承而设计的类应该尽可能少地去实现 Serializable 接口，用户的接口也应该尽可能少继承 Serializable 接口
    5. 内部类不应该实现 Serializable 接口

87. 考虑使用自定义的序列化形式
    1. 如果事先没有认真考虑默认的序列化形式是否合适，则不要贸然接受
    2. 如果一个对象的物理表示等同于它的逻辑内容，可能就适合于使用默认的序列化形式
    3. 即使你确定了默认的序列化形式是合适的，通常还必须提供一个 readObject 方法以保证约束关系和安全性
    4. 当一个对象的物理表示法与它的逻辑数据内容有实质性的区别时，使用默认序列化形式会有以下 4 个缺点：
        1. 它使这个类的导出 API 永远舒服在该类的内部表示法上
        2. 它会消耗过多的空间
        3. 它会消耗过多的时间
        4. 它会引起栈溢出
    5. 如果在读取整个对象状态的任何其他方法上强制任何同步，则也必须在对象序列化上强制这种同步
    6. 不管你选择了哪种序列化形式，都要为自己编写的每个可序列化的类声明一个显式的序列版本UID
    7. 不要修改序列化版本 UID，否则将会破坏现有的已被序列化实例的兼容性

88. 保护性地编写 readObject 方法
    1. 当一个对象被反序列化的时候，对于客户端不应该拥有对象引用，如果哪个域包含了这样的对象引用，就必须要做保护性拷贝，这是非常重要的
    2. 对于任何约束条件，如果检查失败，则抛出一个 InvalidObjectException 异常，这些检查动作应该在所有的保护性拷贝之后
    3. 如果整个对象在被反序列化之后必须进行验证，就应该使用 ObjectInputValidation 接口
    4. 无论是直接方式还是间接方式，都不要调用类中任何被覆盖的方法

89. 对于实例控制，枚举类型优先于 readResolve
    1. 如果在类的声明中加上了 implements Serializable，类就不再是一个单例
    2. readResolve 特性允许你用 readObject 创建的对象代替另一个实例
    3. 如果依赖 readResolve 进行实例控制，带有对象引用类型的所有实例域都必须声明为 transient
    4. 通过将一个可序列化的实例受控的类编写成枚举，Java 可以保证除了所声明的常量之外，不会有其他实例
    5. readResolve 的可访问性很重要

90. 考虑用序列化代理代替序列化实例
    1. 决定实现 Serializable 接口，会增加出错和出现安全问题的可能性
    2. 序列化代理模式相当简单：为可序列化的类设计一个私有的静态嵌套类，其构造器参数类型就是外围类，这样可以精确地表示外围类的实例的逻辑状态
    3. 序列化代理方法可以阻止伪字节流的攻击以及内部域的盗用攻击
